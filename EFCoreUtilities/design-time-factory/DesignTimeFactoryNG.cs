...
namespace J4JSoftware.EFCoreUtilities
{
        ...
        where TDbContext : DbContext
    {
        public DesignTimeFactoryNG()
        {
            ...
            ctors = ctors.Where( x => x.GetParameters()[ 1 ].ParameterType.IsAssignableTo( typeof(IDatabaseConfig) ) ).ToList();
            if( ctors.Count == 0 )
                throw new ArgumentException( $"No constructors with 2nd parameter assignable to {typeof(IDatabaseConfig)}" );
        }

        public TDbContext CreateDbContext( string[] args )
        {
            var dbPath = GetSourceFilePathName();
            dbPath = dbPath == null
                ? args[ 0 ]
                : Path.Combine( Path.GetDirectoryName( dbPath )!, args[ 0 ] );
