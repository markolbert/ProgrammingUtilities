    ...
    public class MonthNumberTickCollection : MinorTickCollection<MonthNumberTick>
    {
        private readonly List<MonthNumberTick> _monthlyTicks = new List<MonthNumberTick>
                                                               {
                                                                   new MonthNumberTick
                                                                   {
                                                                       NormalizedSize = 1, NumberPerMajor = 12
                                                                   },
                                                                   new MonthNumberTick
                                                                   {
                                                                       NormalizedSize = 3, NumberPerMajor = 4
                                                                   },
                                                                   new MonthNumberTick
                                                                   {
                                                                       NormalizedSize = 6, NumberPerMajor = 2
                                                                   },
                                                                   new MonthNumberTick
                                                                   {
                                                                       NormalizedSize = 12, NumberPerMajor = 1
                                                                   },
                                                               };

        public MonthNumberTickCollection( IEnumerable<MonthNumberTick>? yearTicks = null )
        {
            NormalizedRangeTicks = yearTicks?.ToList()
                                   ?? new List<MonthNumberTick>
                                      {
        ...
        public override RangeParameters GetDefaultRangeParameters( double minValue, double maxValue )
        {
            ...
            var minorTick = new MonthNumberTick
                            {
                                NormalizedSize = 1, PowerOfTen = (int) exponent - 1, NumberPerMajor = 12
                            };
